...
if args.function == 'pretrain':
    ...
    trainer_conf = trainer.TrainerConfig(
        ...
        lr_decay=True,
        warmup_tokens=512*20,
        final_tokens=200 * len(pretrain_dataset) * pretrain_dataset.block_size,
        num_workers=4,
        writer=writer
    )

    trainer_o = trainer.Trainer(model, pretrain_dataset, None, trainer_conf)
    trainer_o.train()
    ...
    if args.reading_params_path is not None:
        # This is a pretrained model, finetuning it
        model.load_state_dict(torch.load(args.reading_params_path))
        max_epochs = 10
    else:
        # This is not a pretrained model, training it from scratch
        max_epochs = 75

    text_data = open(args.finetune_corpus_path, encoding='utf-8').read()
    finetune_dataset = dataset.NameDataset(pretrain_dataset, text_data)
    ...
    trainer_o = trainer.Trainer(model, finetune_dataset, None, trainer_conf)
    trainer_o.train()
