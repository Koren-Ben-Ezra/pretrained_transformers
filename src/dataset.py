...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Set a maximum length: at least 4 or roughly 7/8 of the block size, whichever is larger
        max_len = max(4, int(self.block_size * 7/8))

        # Randomly pick a truncation length between 4 and max_len to shorten the doc
        trunc_length = random.randint(4, max_len)
        doc = doc[:trunc_length]
        
        doc_length = max(2, len(doc))
        mask_sz = random.randint(1, doc_length // 2)
        prefix_sz = random.randint(0, doc_length - mask_sz)
        
        # Split the document into three parts:
        # - The prefix (everything before the masked segment)
        # - The chunk we’re going to hide (the masked chunk)
        # - The suffix (everything after the masked segment)
        prefix = doc[:prefix_sz]
        masked_chunk = doc[prefix_sz : prefix_sz + mask_sz]
        suffix = doc[prefix_sz + mask_sz:]

        masked_str = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_chunk

        if len(masked_str) < self.block_size:
            masked_str = masked_str + self.PAD_CHAR * (self.block_size - len(masked_str))
        else:
            masked_str = masked_str[:self.block_size]
